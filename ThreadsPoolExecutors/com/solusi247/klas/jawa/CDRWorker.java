...
//-------------------------------------------------coba kode disini-----------------------------------------------//	
	...
	public void process() throws Exception {
		...
		LineNumberReader lineCounter = new LineNumberReader(new InputStreamReader(new FileInputStream(src_name)));
		...
		try {
			...
			BufferedWriter outputStream = new BufferedWriter(new FileWriter(dest_name));
			...
			while ((nextLine = lineCounter.readLine()) != null) {
				...
				outputStream.write(ss[1] + "|");
				// convert ke unix time
				String str = ss[3];
				DateFormat datef = new SimpleDateFormat("yyyyMMddHHmmss");
				Date date = datef.parse(str);
				outputStream.write(date.getTime() + "|");
				outputStream.write(ss[4] + "|");
				outputStream.write(ss[6]);
	...
	@Override
	public void run() {
		//System.out.println(Thread.currentThread().getName()+src_name+dest_name);
